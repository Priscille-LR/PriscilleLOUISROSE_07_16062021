...
//create map of ingredient/appliance/utensil + ID of recipes used
  private updateMap(filter: string, id: number, map: Map<String, Array<Number>>, selectedTags: Array<string>) {
    ...
    if(!selectedTags.includes(key)) { //check if key isn't contained in selected tags => prevent tag from being selected twice: if tag has been selected, it doesn't appear in the dropdown anymore
      if (map.has(key)) {
